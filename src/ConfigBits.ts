...
export class BitMap {
  #name: string;
  #bits: string;
  #dataType: NumberConstructor | BooleanConstructor;
  /**
   ...
   */
  constructor(name: string, bits: number, dataType: (NumberConstructor | BooleanConstructor) = Number) {
...
export class ConfigBits {
  ...
  constructor(...bitMaps: BitMap[]) {
    ...
    for (let index = 0; index < bitMaps.length; index++) {
      const bitMap: BitMap = bitMaps[index];
   ...
   */
  toObject(integer: number): {[key: string]: number | boolean} {
  ...
  toMap(integer: number): Map<string, number | boolean> {
    ...
    const result: Map<string, number | boolean> = new Map();
    let position = 0;
    for (let index = 0; index < this.#bitMap.length; index++) {
      const bitMap = this.#bitMap[index];
      const value = (integer >> position) & (2 ** bitMap.bits - 1);
   ...
   */
  fromObject(object: {[key: string]: number | boolean}): number {
    ...
    for (let index = 0; index < this.#bitMap.length; index++) {
      const bitMap = this.#bitMap[index];
      const value: number | boolean = object[bitMap.name] || 0;
  ...
  fromMap(map: Map<string, number | boolean>): number {
    ...
    for (let index = 0; index < this.#bitMap.length; index++) {
      const bitMap = this.#bitMap[index];
      const value: number | boolean = map.get(bitMap.name) || 0;
      result |= (+value << position);
      position += bitMap.bits;
    }
    return result;
